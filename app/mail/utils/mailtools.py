...
class MailTool(Mail):
    ...
    def send_mail(self, subject, recipient, template, **kwargs):
        ...
        msg = Message(subject, sender=(self.app.config['MAIL_DEFAULT_SENDER'], self.app.config["MAIL_USERNAME"]),
                      recipients=[recipient])
        msg.html = render_template(template, **kwargs)
        with open(os.path.join(os.getcwd(), "app", 'static', "images", "favicon.ico"), 'rb') as fp:
            msg.attach('favico', 'image/ico', fp.read(), 'inline', headers=[['Content-ID', '<favico>']])
